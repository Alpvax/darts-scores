...
type BuilderNRS<V, K extends string | number = string | number> = {
  withRoundStats: <RS extends TurnStats>(
    factory: (data: FullTurnData<V, K>) => RS,
  ) => BuilderWRS<V, RS, K>;
  withRowClasses: (
    factory: (data: FullTurnData<V, K>[]) => ClassBindings,
  ) => Omit<BuilderNRS<V, K>, "withRoundStats" | "withRowClasses">;
  withCellClasses: (
    factory: (data: FullTurnData<V, K>) => ClassBindings,
  ) => Omit<BuilderNRS<V, K>, "withRoundStats" | "withCellClasses">;
};
type BuilderWRS<V, RS extends TurnStats, K extends string | number = string | number> = {
  withRowClasses: (
    factory: (data: FullTurnDataStats<V, RS, K>[]) => ClassBindings,
  ) => Omit<BuilderWRS<V, RS, K>, "withRowClasses">;
  withCellClasses: (
    factory: (data: FullTurnDataStats<V, RS, K>) => ClassBindings,
  ) => Omit<BuilderWRS<V, RS, K>, "withCellClasses">;
};
export default function builder(meta: GameMetaBase) {
  ...
  function withArrayRounds<V, RS extends TurnStats>(
    rounds: Iterable<RoundWStats<V, RS>>,
  ): BuilderWRS<V, RS, number>;
  function withArrayRounds<V, RS extends TurnStats>(
    rounds: Iterable<RoundNoStats<V>> | Iterable<RoundWStats<V, RS>>,
  ) {}
  return {
    withArrayRounds,
  };

...
const Game27 = createComponent({
  positionOrder: "highestFirst",
  startScore: () => 27,
  rounds: arrayRoundsBuilder<number>((i) => ({
    label: (i + 1).toString(),
    display: ({ score, deltaScore, value: hits, editable, focus }) => (
      <>
        <span>{score}</span>
        <sup>({numfmt.format(deltaScore)})</sup>
        {editable ? (
          <input
            class="hitsInput"
            type="number"

...
const applyTrap: ProxyHandler<object>["apply"] = () => { throw new Error("Unsupported operation") }

// A trap for the new operator.
const constructTrap: ProxyHandler<object>["construct"] = () => { throw new Error("Unsupported operation") }

// A trap for Object.defineProperty.
const definePropertyTrap: ProxyHandler<object>["defineProperty"] = () => { throw new Error("As yet Unsupported operation") }

// A trap for the delete operator.
const deletePropertyTrap: ProxyHandler<object>["deleteProperty"] = () => { throw new Error("As yet Unsupported operation") }
...
const getOwnPropertyDescriptorTrap: ProxyHandler<object>["getOwnPropertyDescriptor"] = () => { throw new Error("As yet Unsupported operation") }

// A trap for Object.getPrototypeOf.
const getPrototypeOfTrap: ProxyHandler<object>["getPrototypeOf"] = () => { throw new Error("As yet Unsupported operation") }
...
const isExtensibleTrap: ProxyHandler<object>["isExtensible"] = () => { throw new Error("As yet Unsupported operation") }

// A trap for Object.getOwnPropertyNames and Object.getOwnPropertySymbols.
const getKeysRecursive = <T extends Record<string, any>>(obj: T, prefix?: string): string[] => {
  let keys = Object.getOwnPropertyNames(obj).flatMap((k: keyof T & string) => typeof obj[k] === "object" && !Array.isArray(obj[k])
...
const preventExtensionsTrap: ProxyHandler<object>["preventExtensions"] = () => { throw new Error("Unsupported operation") }

// A trap for setting property values.
const setTrap: ProxyHandler<object>["set"] = () => { throw new Error("As yet Unsupported operation") }

// A trap for Object.setPrototypeOf.
const setPrototypeOfTrap: ProxyHandler<object>["setPrototypeOf"] = () => { throw new Error("Unsupported operation") }
...
export const flattenedAccessProxy = <T extends object>(obj: T): Flatten<T> => nestedAccessProxy(obj) as Flatten<T>;
  ...
  : (T & {
    [K in keyof T]-?: (x: NonNullable<T[K]> extends infer V
        ? V extends object
          ? V extends any[]
            ? Pick<T, K>
            : Pick<T, K> & (NestedKeys<V> extends infer FV
              ? ({

import { computed, onMounted, onUnmounted, reactive, ref, type ComputedRef, type Ref, type WritableComputedRef } from "vue";
...
const makeMergeFunc = <V>(merge: MergeOption<V>): MergeFunction<V> => {
  if (typeof merge === "string") {
    switch (merge) {
      case "replace":
        return (partial, fallback) => ([StorageLocation.Volatile, StorageLocation.Session, StorageLocation.Local].map(l => partial.get(l)).find(v => v) ?? fallback) as V;
    }
  }
  return merge;
};
...
type ParseOption<V> = ((s: string) => V extends object ? NestedPartial<V> | undefined : V | undefined) | "rawString" | "json";
...
const makeBrowserRef = <V>(storageKey: string, meta: BrowserStorageValue<V>, recalculateFallback = false): ConfigRef<V> => {
  ...
  const fallback = typeof meta.fallback === "function"
    ? recalculateFallback
      ? () => {
  ...
  return Object.defineProperty({
    ...
    readonlyRef: (): ComputedRef<V> => {
      ...
      if (!refCache.has(null)) {
        refCache.set(null, computed(() => merge(storageLayers.value, fallback())));
      }
    ...
    mutableRef: (storageLocation: StorageLocation): WritableComputedRef<V> => {
      ...
      if (!refCache.has(storageLocation)) {
        let set: (val: V) => void;
        switch (storageLocation) {
          ...
          case StorageLocation.Session:
            set = (val) => {
              setVolatile(val);
              sessionStorage.setItem(storageKey, ((meta as BrowserStorageValue<V>).toStr ?? JSON.stringify)(val));
            };
            break;
          case StorageLocation.Local:
            set = (val) => {
              setVolatile(val);
              localStorage.setItem(storageKey, ((meta as BrowserStorageValue<V>).toStr ?? JSON.stringify)(val));
            };
            break;
        }
        refCache.set(storageLocation, computed({
          get: () => merge(storageLayers.value, fallback()),
          set, 
        }));
      }
      return refCache.get(storageLocation)!;
    },
...
const isConfigRef = (val: any): val is ConfigRef<any> => val !== null && typeof val === "object" && CONFIG_REF_IDENTIFIER in val;
...
export function makeConfigComposable<Values extends { [k: string]: StorageValue<any> }>(values: Values): () => StorageRefs<Values>;
export function makeConfigComposable<Values extends { [k: string]: StorageValue<any> }>(prefix: string, values: Values): () => StorageRefs<Values>;
export function makeConfigComposable<Values extends { [k: string]: StorageValue<any> }>(prefixOrValues: string | Values, maybeValues?: Values): () => StorageRefs<Values> {
  const [prefix, values] = typeof prefixOrValues === "string"
    ? [prefixOrValues, maybeValues!]
    : [undefined, prefixOrValues];
  const unparsedValues = reactive(new Map<string, UnparsedValues>());
  const refs = Object.freeze(Object.entries(values).reduce((refs, [key, meta]) => {
    const unparsed: UnparsedValues = {};
    switch (meta.location) {
      case StorageLocation.Volatile:
        (refs as any)[key] = ref(typeof meta.fallback === "function"
          ? (meta.fallback as () => void)()
          : meta.fallback);
        break;
      case StorageLocation.Local:
    ...
    return refs;
  }, {} as StorageRefs<Values>)) as StorageRefs<Values>;
  const handle = (event: StorageEvent) => {
    const key = prefix
      ? event.key?.startsWith(prefix) ? event.key.substring(prefix.length) : undefined
      : event.key;
    if (key && key in refs) {
      if (!unparsedValues.has(key)) {

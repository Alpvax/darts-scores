<template>
  <div class="gameViewBody">
    <PlayerSelection
      :model-value="players.map(({id}) => id)"
      legend="Select who is playing:"
      :available="all_players"
      :allow-guests="allowGuestSelection"
      @update:model-value="p => players = all_players.filter(({ id }) => p.includes(id))"
    />
    <input
      id="dateInput"
      v-model="date"
    ...
    <div class="game">
      <Game27
        :players="players /*all_players.filter(({ id }) => players.includes(id))*/"
        :date="new Date(date)"
      />
    </div>
    <!-- <Summary27
...
import Game27 from "@/components/27/v2/playerHits";
...
export default defineComponent({
  ...
  async setup() {
    ...
    // const playerStoreOD = onDemandPlayerStore();
    ...
    const players = ref((await playerStore.getDefaultPlayers("twentyseven")).map(r => r.value));
    // playerStore.getDefaultPlayers("twentyseven").then((res) => {
    //   console.log("Loaded players:", res.map(r => r.value));//XXX
    //   players.value = res.map(r => r.value);//res.map(({ value:{ id }}) => id);
    // });
    return {
      players,
      all_players,
      ...
      summaryPlayers: computed(() => {
        switch (preferences.twentyseven.ingameSummary.players) {
          ...
          case "playing":
            return history.summaryPlayers.filter(({ id }) => players.value.find(p => p.id === id) !== undefined);
